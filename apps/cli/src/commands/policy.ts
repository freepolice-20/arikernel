...
import { PRESETS, getPreset, type PresetId } from '@arikernel/core';
...
export function runPolicyList(): void {
	...
	for (const [id, preset] of Object.entries(PRESETS)) {
		...
		console.log(`    Capabilities: ${preset.capabilities.length}  Policies: ${preset.policies.length}  Quarantine threshold: ${threshold}`);
		console.log();
	}
}
	...
	try {
		...
		for (const cap of preset.capabilities) {
			const constraints = cap.constraints
				? ` [${Object.entries(cap.constraints).map(([k, v]) => `${k}: ${JSON.stringify(v)}`).join(', ')}]`
				: '';
		...
		if (preset.runStatePolicy) {
			console.log('\nRun State Policy:');
			console.log(`  Max denied sensitive actions: ${preset.runStatePolicy.maxDeniedSensitiveActions ?? '—'}`);
