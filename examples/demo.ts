...
import { type TaintLabel, ToolCallDeniedError } from "@arikernel/core";
...
async function main() {
	...
	const firewall = createFirewall({
		...
		hooks: {
			onDecision(toolCall, decision) {
				const color =
					decision.verdict === "allow" ? GREEN : decision.verdict === "deny" ? RED : YELLOW;
	...
	console.log(
		`  ${DIM}Capability: ${httpGrant.granted ? `${GREEN}GRANTED` : `${RED}DENIED`}${RESET}`,
	);
	...
	console.log(
		`  ${DIM}Capability: ${shellGrant.granted ? `${GREEN}GRANTED` : `${RED}DENIED`}${RESET}`,
	);
