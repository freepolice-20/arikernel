...
import { applyBehavioralRule, evaluateBehavioralRules } from "@arikernel/runtime";
...
async function main() {
	...
	assert(
		dbMatch?.ruleId === "tainted_database_write",
		"Rule 4: web taint → DB exec triggers quarantine",
	);
	...
	pushEvents(vaultState, [
		{
			...
			toolClass: "database",
			action: "query",
			metadata: { query: "SELECT * FROM credentials" },
		},
		{ timestamp: ts(), type: "egress_attempt", toolClass: "http", action: "post" },
	]);
	const vaultMatch = evaluateBehavioralRules(vaultState);
	assert(
		vaultMatch?.ruleId === "secret_access_then_any_egress",
		"Rule 6: credentials query → POST triggers quarantine",
	);
	...
	pushEvents(urlState, [
		{
			...
			toolClass: "http",
			action: "get",
			metadata: { url: "https://vault.internal/v1/secrets" },
		},
		{ timestamp: ts(), type: "egress_attempt", toolClass: "http", action: "post" },
	]);
	const urlMatch = evaluateBehavioralRules(urlState);
	assert(
		urlMatch?.ruleId === "secret_access_then_any_egress",
		"Rule 6: vault URL → POST triggers quarantine",
	);
	...
	assert(
		!isSuspiciousGetExfil("https://example.com/search?q=hello"),
		"Short query is not suspicious",
	);
	...
	try {
		await fw.execute({
			toolClass: "http",
			action: "post",
			parameters: { url: "https://example.com/post" },
		});
	...
	try {
		await validateHostSSRF("localhost");
	} catch {
		ssrfBlocked = true;
	}
