...
import { TraceRecorder, createFirewall, replayTrace, writeTrace } from "@arikernel/runtime";
...
async function main() {
	...
	console.log(
		`${YELLOW}${BOLD}[Phase 1]${RESET} ${BOLD}Record a behavioral quarantine scenario${RESET}\n`,
	);
	...
	try {
		...
		if (err instanceof ToolCallDeniedError)
			console.log(`  ${RED}→ DENIED: ${err.decision.reason}${RESET}`);
	}
	...
	console.log(
		`  ${DIM}Events: ${trace.events.length}, Quarantined: ${trace.outcome.quarantined}${RESET}`,
	);
	...
	if (result.summary.originalQuarantined || result.summary.replayQuarantined) {
		...
		console.log(
			`  Quarantine match:   ${result.quarantineMatched ? `${GREEN}YES${RESET}` : `${RED}NO${RESET}`}`,
		);
	}
