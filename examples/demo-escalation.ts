...
async function main() {
	...
	info(`Capability issuance: ${httpGrant.granted ? `${GREEN}GRANTED${RESET}` : `${RED}DENIED${RESET}`}`);

	if (httpGrant.granted) {
		info(`Token: ${httpGrant.grant!.id.slice(0, 12)}...`);
		info(`Lease: ${httpGrant.grant!.lease.maxCalls} calls, expires ${httpGrant.grant!.lease.expiresAt}\n`);
	}
	...
	try {
		...
		console.log(`  ${DIM}Status: ${(result.data as any)?.status ?? 'ok'}, Duration: ${result.durationMs}ms${RESET}\n`);
		allowed++;
	} catch (err) {
		if (err instanceof ToolCallDeniedError) {
			console.log(`  ${BG_RED}${WHITE}${BOLD} BLOCKED ${RESET} ${RED}HTTP GET (unexpected)${RESET}`);
	...
	try {
		...
		console.log(`  ${BG_GREEN}${WHITE}${BOLD} ALLOWED ${RESET} ${RED}POST succeeded — escalation not blocked!${RESET}\n`);
		allowed++;
	} catch (err) {
		if (err instanceof ToolCallDeniedError) {
			console.log(`  ${BG_RED}${WHITE}${BOLD} BLOCKED ${RESET} ${RED}HTTP POST with GET-only token${RESET}`);
			console.log(`  ${DIM}${err.decision.reason}${RESET}\n`);
			denied++;
		}
	}
	...
	info(`Capability issuance: ${shellIssuance.granted ? `${GREEN}GRANTED${RESET}` : `${RED}DENIED${RESET}`}`);
	...
	try {
		...
		console.log(`  ${BG_GREEN}${WHITE}${BOLD} ALLOWED ${RESET} ${RED}Shell exec succeeded — escalation not blocked!${RESET}\n`);
		allowed++;
	} catch (err) {
		if (err instanceof ToolCallDeniedError) {
			console.log(`  ${BG_RED}${WHITE}${BOLD} BLOCKED ${RESET} ${RED}Shell execution without capability${RESET}`);
			console.log(`  ${DIM}${err.decision.reason}${RESET}\n`);
			denied++;
		}
	}
	...
	info(`Capability issuance: ${fileGrant.granted ? `${GREEN}GRANTED${RESET}` : `${RED}DENIED${RESET}`}`);

	if (fileGrant.granted) {
		info(`Token: ${fileGrant.grant!.id.slice(0, 12)}...`);
		info(`Constraint: allowedPaths = ${JSON.stringify(fileGrant.grant!.constraints.allowedPaths)}\n`);
	}
	...
	try {
		...
		console.log(`  ${BG_GREEN}${WHITE}${BOLD} ALLOWED ${RESET} ${RED}File read succeeded — constraint bypass!${RESET}\n`);
		allowed++;
	} catch (err) {
		if (err instanceof ToolCallDeniedError) {
			console.log(`  ${BG_RED}${WHITE}${BOLD} BLOCKED ${RESET} ${RED}File read outside allowed paths${RESET}`);
			console.log(`  ${DIM}${err.decision.reason}${RESET}\n`);
			denied++;
		}
	}
	...
	try {
		...
		console.log(`  ${BG_GREEN}${WHITE}${BOLD} ALLOWED ${RESET} ${RED}Revoked token accepted — lifecycle broken!${RESET}\n`);
		allowed++;
	} catch (err) {
		if (err instanceof ToolCallDeniedError) {
			console.log(`  ${BG_RED}${WHITE}${BOLD} BLOCKED ${RESET} ${RED}Revoked token rejected${RESET}`);
			console.log(`  ${DIM}${err.decision.reason}${RESET}\n`);
			denied++;
		}
	}
	...
	console.log(`  ${GREEN}${BOLD}Allowed:${RESET} ${allowed} action(s)  ${DIM}(legitimate use within granted scope)${RESET}`);
	console.log(`  ${RED}${BOLD}Denied:${RESET}  ${denied} action(s)  ${DIM}(escalation attempts blocked)${RESET}\n`);

	if (denied === 4) {
		console.log(`  ${BG_GREEN}${WHITE}${BOLD} ALL 4 ESCALATION ATTEMPTS BLOCKED ${RESET}\n`);
	} else {
		console.log(`  ${BG_RED}${WHITE}${BOLD} ${4 - denied}/4 ESCALATION ATTEMPTS SUCCEEDED ${RESET}\n`);
	}
	...
	console.log(`The audit trail contains ${events.length} events — 1 allowed, ${denied} denied.${RESET}\n`);
