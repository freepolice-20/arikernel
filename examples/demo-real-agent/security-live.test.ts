...
async function runAgent(
	...
	toolCalls: Array<{ name: string; args: Record<string, unknown>; denied: boolean; reason?: string }>;
	finalMessage: string;
}> {
	...
	const toolCalls: Array<{ name: string; args: Record<string, unknown>; denied: boolean; reason?: string }> = [];
	...
	it('blocks the full attack sequence: fetch → read ssh key → quarantine → block POST', async () => {
		fw = createFirewall({
			principal: {
				...
				capabilities: [
					{ toolClass: 'http', actions: ['get', 'post'], constraints: { allowedHosts: ['corp-reports.internal'] } },
...
describe.skipIf(!API_KEY)('Live: Quarantine preserves GET ingress', () => {
	...
	it('allows http.read capability after quarantine, blocks http.write', async () => {
		...
		for (const path of ['~/.ssh/id_rsa', '~/.aws/credentials']) {
			try { await fw.execute({ toolClass: 'file', action: 'read', parameters: { path }, grantId: grant.grant!.id }); } catch {}
		}
		expect(fw.isRestricted).toBe(true);
