...
async function main() {
	...
	info(
		`Capability http.read: ${httpGrant.granted ? `${GREEN}GRANTED${RESET}` : `${RED}DENIED${RESET}`}`,
	);
	...
	try {
		...
		console.log(
			`  ${BG_GREEN}${WHITE}${BOLD} ALLOWED ${RESET} ${GREEN}HTTP GET api.github.com${RESET}`,
		);
	} catch (err) {
	...
	info(
		`Restricted: ${firewall.isRestricted}, Denied: ${firewall.runStateCounters.deniedActions}\n`,
	);
	...
	try {
		...
		console.log(
			`  ${BG_GREEN}${WHITE}${BOLD} ALLOWED ${RESET} ${GREEN}HTTP GET with web taint${RESET}`,
		);
	} catch (err) {
		if (err instanceof ToolCallDeniedError) {
			console.log(
	...
	info(
		`Capability file.read: ${fileGrant.granted ? `${GREEN}GRANTED${RESET}` : `${RED}DENIED${RESET}`}`,
	);
	...
	try {
		...
		console.log(
			`  ${BG_GREEN}${WHITE}${BOLD} ALLOWED ${RESET} ${GREEN}file.read ~/.ssh/id_rsa${RESET}`,
		);
	} catch (err) {
		if (err instanceof ToolCallDeniedError) {
			console.log(
				`  ${BG_RED}${WHITE}${BOLD} BLOCKED ${RESET} ${RED}file.read ~/.ssh/id_rsa${RESET}`,
			);
	...
	if (firewall.isRestricted) {
		...
		console.log(
			`  ${MAGENTA}Only ${firewall.runStateCounters.deniedActions} denied actions — threshold was 10!${RESET}\n`,
		);
	} else {
	...
	info(
		`Capability http.write: ${writeGrant.granted ? `${GREEN}GRANTED${RESET}` : `${RED}DENIED${RESET}`}`,
	);
	...
	try {
		...
		if (err instanceof ToolCallDeniedError) {
			console.log(
				`  ${BG_RED}${WHITE}${BOLD} BLOCKED ${RESET} ${RED}HTTP POST blocked by quarantine${RESET}`,
			);
		}
	}
	console.log("");
