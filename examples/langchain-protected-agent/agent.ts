...
async function runAgent() {
	...
	try {
		...
		console.log(`  ${GREEN}${BOLD}ALLOWED${RESET} ${DIM}— page fetched, content returned to agent${RESET}\n`);
	} catch (err) {
		printDenied(err);
	}
	...
	console.log(`${YELLOW}${BOLD}Step 2${RESET} ${BOLD}Agent processes page with embedded injection${RESET}`);
	...
	const webTaint: TaintLabel[] = [
		{ source: 'web', origin: 'example.com/research', confidence: 0.9, addedAt: new Date().toISOString() },
	];
	const taintedFetch = wrapTool(firewall, 'http', 'get', { taintLabels: webTaint });

	try {
		await taintedFetch({ url: 'https://example.com/injected-link' });
		console.log(`  ${GREEN}${BOLD}ALLOWED${RESET} ${DIM}— taint label "web" now tracked by runtime${RESET}\n`);
	} catch (err) {
		printDenied(err);
	}
	...
	console.log(`${YELLOW}${BOLD}Step 3${RESET} ${BOLD}Agent reads ~/.ssh/id_rsa (injected instruction)${RESET}`);
	...
	console.log(`${YELLOW}${BOLD}Step 4${RESET} ${BOLD}Agent reads a safe file (quarantine allows read-only)${RESET}`);
	...
	console.log(`${CYAN}${BOLD}${'═'.repeat(60)}${RESET}`);
	console.log(`  ${GREEN}${allowed} allowed${RESET}  ${RED}${denied} denied${RESET}  ${MAGENTA}${quarantine} quarantine${RESET}`);
