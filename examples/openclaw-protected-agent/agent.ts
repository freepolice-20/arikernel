...
async function main() {
	const firewall = createFirewall({
		...
		policies: [
			...
			{ id: 'allow-db-query', name: 'Allow DB queries', priority: 100,
				match: { toolClass: 'database' as const, action: 'query' }, decision: 'allow' as const },
		],
		auditLog: ':memory:',
	...
	const adapter = new OpenClawAdapter(firewall)
		.registerSkill('web_search', 'http', 'get',
			(args) => `Search results for "${args.query}" from ${args.url}`,
			{ description: 'Search the web via API' })
		.registerSkill('read_document', 'file', 'read',
			(args) => `Document contents: [${args.path}]`,
			{ description: 'Read a document from workspace' })
		.registerSkill('query_db', 'database', 'query',
			(args) => `Query result: 42 rows from ${args.table}`,
			{ description: 'Query the database' })
		.registerSkill('run_command', 'shell', 'exec',
			(args) => `Output of: ${args.cmd}`,
	...
	try {
		const result = await adapter.executeSkill('web_search', {
			query: 'AriKernel security', url: 'https://api.example.com/search',
		});
