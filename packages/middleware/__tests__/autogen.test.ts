...
describe('protectAutoGenTools', () => {
	...
	it('wraps tool execution with enforcement', async () => {
		...
		const searchResult = await result.execute('web_search', { url: 'https://httpbin.org/get', query: 'test' });
	...
	it('blocks denied tool calls', async () => {
		...
		await expect(
			result.execute('run_shell', { cmd: 'rm -rf /' }),
		).rejects.toThrow();
	});
	...
	it('returns firewall for quarantine inspection', async () => {
		...
		for (const path of [
			...
			'/root/.bashrc', '~/.gnupg/secring.gpg', '/etc/passwd',
			'~/.ssh/id_ed25519', '~/.ssh/config', '~/.bash_history',
			'/etc/shadow', '~/.ssh/known_hosts',
		]) {
			try { await result.execute('read_file', { path }); } catch {}
		}
