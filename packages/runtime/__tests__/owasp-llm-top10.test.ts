...
import { createFirewall, type Firewall, RunStateTracker, createSecretPatternFilter } from '../src/index.js';
...
describe('LLM01 — Prompt Injection', () => {
	...
	it('RAG-tainted input followed by file write is blocked by policy', async () => {
		...
		try {
			...
			await fw.execute({ toolClass: 'file', action: 'write', parameters: { path: './data/out.txt' } });
			expect.unreachable();
		} catch (err) {
...
describe('LLM02 — Insecure Output Handling (DLP)', () => {
	...
	const tc = {
		...
		principalId: 'test', toolClass: 'http', action: 'get',
...
describe('LLM06 — Sensitive Information Disclosure', () => {
	...
	it('vault query then egress triggers quarantine', () => {
		const state = new RunStateTracker();
		pushEvents(state, [
			{ timestamp: ts(), type: 'tool_call_allowed', toolClass: 'database', action: 'query', metadata: { query: 'SELECT api_key FROM secrets' } },
			{ timestamp: ts(), type: 'egress_attempt', toolClass: 'http', action: 'post' },
		]);
		const match = evaluateBehavioralRules(state);
		expect(match).not.toBeNull();
