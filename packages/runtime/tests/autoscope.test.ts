...
describe('classifyScope', () => {
	...
	it('classifies document tasks as rag-reader', () => {
		...
		expect(classifyScope('parse and extract data from this PDF document').preset).toBe('rag-reader');
	...
	it('classifies coding tasks as workspace-assistant', () => {
		expect(classifyScope('refactor the code in this repository').preset).toBe('workspace-assistant');
		expect(classifyScope('fix the bug in the test suite').preset).toBe('workspace-assistant');
		expect(classifyScope('implement a new feature and write tests').preset).toBe('workspace-assistant');
		expect(classifyScope('create a new file for the build system').preset).toBe('workspace-assistant');
	});
	...
	it('classifies automation tasks as automation-agent', () => {
		...
		expect(classifyScope('send email notification and update database records').preset).toBe('automation-agent');
	});
