...
describe('validateCommand', () => {
	...
	it('rejects command chaining with ;', () => {
		expect(validateCommand('ls; rm -rf /', ALLOWED)).toContain(
			'dangerous shell metacharacter',
		);
	});
	...
	it('rejects backtick injection', () => {
		expect(validateCommand('echo `whoami`', ALLOWED)).toContain(
			'dangerous shell metacharacter',
		);
	});
	...
	it('rejects variable expansion', () => {
		expect(validateCommand('echo $HOME', ALLOWED)).toContain(
			'dangerous shell metacharacter',
		);
	});
