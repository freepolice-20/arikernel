...
import { TraceRecorder, writeTrace, readTrace } from '../src/trace-recorder.js';
...
async function recordQuarantineTrace(): Promise<ReplayTrace> {
	...
	try {
		await firewall.execute({
			...
			taintLabels: [{ source: 'web', origin: 'example.com', confidence: 0.9, addedAt: new Date().toISOString() }],
		});
	} catch {}
...
describe('TraceRecorder', () => {
	...
	it('captures tool call request details', async () => {
		const trace = await recordQuarantineTrace();
		const httpEvent = trace.events.find((e) => e.request.toolClass === 'http' && e.request.action === 'get');
