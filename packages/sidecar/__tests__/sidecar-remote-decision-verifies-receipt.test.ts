...
/** Build a valid signed receipt using the test signer. */
function signedReceipt(verdict: "allow" | "deny" = "allow") {
	return SIGNER.sign({
		decision: verdict,
		reason: "test-reason",
		policyVersion: "1.0.0",
...
describe("DecisionDelegate — receipt verification", () => {
	...
	it("accepts a valid signed receipt", async () => {
		const mock = await startMockCP(19200, () => signedReceipt("allow"));
		servers.push(mock);
	...
	it("rejects a receipt with a tampered signature", async () => {
		const mock = await startMockCP(19201, () => {
			const receipt = signedReceipt("allow");
	...
	it("rejects a receipt with a tampered reason field", async () => {
		const mock = await startMockCP(19202, () => {
			const receipt = signedReceipt("deny");
	...
	it("rejects a replayed nonce (same receipt sent twice)", async () => {
		const receipt = signedReceipt("allow");
		const mock = await startMockCP(19203, () => receipt);
		servers.push(mock);
	...
	it("rejects a receipt signed by a different key", async () => {
		...
		const mock = await startMockCP(19204, () =>
			otherSigner.sign({
				decision: "allow",
				reason: "wrong-key",
				policyVersion: "1.0.0",
	...
	it("skips verification when no public key is configured", async () => {
		const mock = await startMockCP(19205, () => ({
			decision: "allow",
			reason: "no-verify",
	...
	it("returns deny verdicts after successful verification", async () => {
		const mock = await startMockCP(19206, () => signedReceipt("deny"));
		servers.push(mock);
