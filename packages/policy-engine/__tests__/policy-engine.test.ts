...
import { PolicyEngine, matchesRule, DENY_ALL_RULE, DEFAULT_RULES } from '../src/index.js';
...
describe('PolicyEngine', () => {
	...
	it('returns require-approval verdict when rule specifies it', () => {
		...
		const tc = makeToolCall({ toolClass: 'shell', action: 'exec' });
		const decision = engine.evaluate(tc, [], [makeCapability({ toolClass: 'shell', actions: ['exec'] })]);
