...
import { AuditStore, computeHash, genesisHash, verifyChain, replayRun, verifyDatabaseChain } from '../src/index.js';
...
describe('verifyChain', () => {
	...
	it('detects tampered hash', () => {
		...
		const result = verifyChain([
			{ hash: 'tampered_hash', previousHash: genesis, data: data1 },
		]);
...
describe('AuditStore', () => {
	...
	it('appendSystemEvent creates _system event', () => {
		...
		const event = store.appendSystemEvent('run-1', 'agent', 'quarantine', 'Behavioral trigger', { ruleId: 'test' });
