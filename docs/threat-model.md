...
| **Cross-principal collusion** | Two principals coordinating an attack (e.g., one reads secrets, the other exfiltrates) are not detected unless the exfiltrating principal independently triggers behavioral rules. In sidecar mode, each principal has a completely isolated `RunStateTracker`, `TaintTracker`, and `AuditStore` — taint and sticky flags do not propagate across principals. **Mitigation**: (1) Minimize capability overlap — avoid granting both `file.read` and `http.write` to principals that share data channels. (2) Deploy an external log correlator to detect cross-principal patterns (e.g., "A reads secrets, B exfiltrates within N minutes") by analyzing per-principal audit DBs. (3) Use container/VM isolation with network policy to restrict covert channels between agent processes. |
